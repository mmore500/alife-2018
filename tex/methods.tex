...
% @CAO: I changed the -8.0 to just 8.0 since you already specified "cost."
...
The cell at the starting position of a resource wave is automatically activated, and will send the activate signal to neighboring cells on the same signaling channel. %, so that they activate on the next update.  % to enter the activated state and itself transitions to a temporary quiescent state.
...
In this manner, cells sharing a signaling channel activate %coincident 
along with the expanding resource wave.
As shown Figure \ref{fig:explanatory}$a,b$, the rate of resource collection for a cell is determined by the size and shape of of its %same-channel  %@CAO: I realize that "same-channel" is relevent in the code, but we haven't explained the linking between types of resource and signaling channels yet...
signaling network;
small or fragmented %same-channel
signaling networks will frequently miss out on resource as it passes by.

Each cell pays a resource cost when it activates. %enters the activated state.
...
In this manner, ``Goldilocks'' --- not to small and not too big --- %same-channel 
...
This process ensures that selection for ``Goldilocks'' %same-channel
signaling networks is uniformly distributed over the toroidal grid.

Cells control the size and shape of their %same-channel
signaling group by strategicly reproducing. % control of reproduction.
Three choices are afforded: whether to reproduce at all, where among the four adjoining tiles of the toroidal grid to place their offspring, and whether the offspring should be registered to the parent's signaling channel or %should instead be registered to
be given
...
% @CAO: HERE is where we first introduce how multiple channels work; only from here on do we need to worry about talking about "same-channel signalling"
Hierarchical levels are introduced into the system through multiple, 
% @CAO: I don't think independent is quite the right word here - probably nested?
%independent, but overlaid, 
...
The \textbf{aversion parameters} ($A_1$ and $A_2$) allow cells to avoid reproducing over neighbors sharing the same signaling channel.
...
If a cell declines to place its offspring in all four adjoining tiles, it decides not to %pay the resource cost to reproduce and does not 
reproduce.
...
%@CAO: I assume you make sure they're not all 0?  You don't need to say anything about that here.
...
The \textbf{endowment parameters} $E_{c}$, $E_1$, and $E_2$ determine the amount of resource provided to offspring.
This endowment is paid as an additional cost by the cell stockpile (or same-channel resource pool) funding a reproduction.
The full amount of the received endowment is divided between the daughter cell's stockpile, level-one same-channel resource pool, and level-two same-channel resource pool according to the offspring's resource allocation parameters. % $P_{c}$, $P_1$, and $P_2$.
%Specifically, 
...
To assess the relative fitness of these evolved organisms, we ran %ecological
