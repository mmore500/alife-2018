...
In order to collect resource as it passes overhead, cells must enter an activated state from their resting ready state.
The cell at the starting position of a resource wave is automatically set to the activated state from the ready state.
...
Subsequently, the newly activated cell activate ready neighbors registered to the same signaling channel and themselves enter a temporary quiescent state.
...
Each cell pays a resource cost when it enters the activated state.
